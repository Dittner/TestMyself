...
public class SelectTestTasksSQLOperation extends AsyncOperation implements IAsyncCommand {
	...
	public function SelectTestTasksSQLOperation(service:SQLStorage, spec:TestSpec) {
		super();
	...
	public function execute():void {
		if (spec) {
			...
			if (filter.selectedThemes.length > 0) {
				sql = info.useExamples ? service.sqlFactory.selectFilteredTestExampleTask : service.sqlFactory.selectFilteredTestTask;
				var themes:String = SQLUtils.themesToSqlStr(filter.selectedThemes);
				sql = sql.replace("#filterList", themes);
			}
			else {
				sql = info.useExamples ? service.sqlFactory.selectTestExampleTask : service.sqlFactory.selectTestTask;
			}
