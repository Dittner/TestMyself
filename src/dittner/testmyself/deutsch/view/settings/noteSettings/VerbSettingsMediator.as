...
public class VerbSettingsMediator extends SFMediator {
	...
	private function applyBtnClickHandler(event:MouseEvent):void {
		...
		if (view.isThemeRemoving) {
			sendRequestTo(ModuleName.VERB, NoteMsg.REMOVE_THEME, new RequestMessage(null, null, selectedTheme))
		}
			...
			if (duplicate) sendRequestTo(ModuleName.VERB, NoteMsg.MERGE_THEMES, new RequestMessage(null, null, {destTheme: duplicate, srcTheme: selectedTheme}));
			else sendRequestTo(ModuleName.VERB, NoteMsg.UPDATE_THEME, new RequestMessage(null, null, theme));
		}
	}
