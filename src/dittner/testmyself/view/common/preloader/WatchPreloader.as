...
public final class WatchPreloader extends SpriteAsset implements IPreloaderDisplay {

	[Embed(source='/swf/watchBusyIndicator.swf')]
	private var WatchClass:Class;

	private static const MINIMUM_DISPLAY_TIME:uint = 3000;

	public function WatchPreloader(color:uint = 0xFFffFF):void {
	...
	private function updateWatchPos():void {
		if (watch) {
			watch.x = (fullScreenWidth - watch.width) / 2;
			watch.y = (fullScreenHeight - watch.height) / 2;
			/*var g:Graphics = graphics;
			var borderWidth:Number = 50;
			g.clear();
			g.beginFill(0x515368, .5);
			g.drawRect(watch.x - borderWidth, watch.y - borderWidth, watch.width + 2 * borderWidth, watch.height + 2 * borderWidth);
			g.endFill();*/
		}
	}
